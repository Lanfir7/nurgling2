...
public abstract class UILoop implements Console.Directory {
    ...
    /* nurgling: overridable factory so subclasses (client/headless) can build NUI
     * instead of plain UI without duplicating the swap logic below. */
    ...
    /* next: F8 toggles rendering (was GLPanel.Loop.renderDisabled). */
	...
	UI prevui, newui;
	/* nurgling: a listener may supply an existing UI to reuse (session switch). */
	UI reuse = (lifecycleListener != null) ? lifecycleListener.beforeNewUI(fun, this.ui, this) : null;
	if(reuse != null) {
	    newui = reuse;
	    newui.env = this.env;
	} else {
	...
	/* nurgling: listener decides whether the old UI is destroyed or demoted. */
	boolean destroyOld = (lifecycleListener == null) || lifecycleListener.afterNewUI(newui, prevui);
	if((prevui != null) && (prevui != newui) && destroyOld) {
	    prevui.destroy();
	}
	return(newui);
    }
