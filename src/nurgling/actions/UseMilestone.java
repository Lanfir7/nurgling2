...
public class UseMilestone implements Action {
    ...
    public enum Arrival { OK, UNRESOLVED, WRONG_PLACE }

    public static double wrongLocationTolerance() {
        return WRONG_LOCATION_TOLERANCE;
    }

    /**
     * Dest world coords are often still null after a cross-segment teleport (mmap.sessloc has not
     * caught up). That must not be treated as "landed in the wrong place" or Forager hearths home.
     */
    public static Arrival classifyArrival(Coord2d expectedWorld, Coord2d playerRc, double tolerance) {
        if (expectedWorld == null || playerRc == null) {
            return Arrival.UNRESOLVED;
        }
        return playerRc.dist(expectedWorld) > tolerance ? Arrival.WRONG_PLACE : Arrival.OK;
    }

    /** Peek-bail already ran the guard outcome; SUCCESS would let Forager keep walking from home. */
    static Results resultAfterPeekBail() {
        return Results.FAIL();
    }

    /** After a confirmed wrong landing, stop the route even if hearth travel itself succeeded. */
    public static Results resultAfterConfirmedWrongPlace(Results hearthResult) {
        if (hearthResult != null && !hearthResult.IsSuccess()) {
            return hearthResult;
        }
        return Results.FAIL();
    }
    ...
    public Results run(NGameUI gui) throws InterruptedException {
        ...
        if (expectedDestination != null) {
            ...
            if (classifyArrival(expectedWorld, playerRc, WRONG_LOCATION_TOLERANCE) == Arrival.WRONG_PLACE) {
                ...
                return resultAfterConfirmedWrongPlace(new TravelToHearthFire().run(gui));
            }
        }
