...
public class MinesweeperMiner implements Action {
     ...
     */
    static boolean latchCaveGallery(NNoticeLog notices, long since, boolean alreadyLatched) {
        if (alreadyLatched)
            return true;
        if (notices == null)
            return false;
        return notices.contains(since, CAVE_GALLERY_NOTICE);
    }

    private boolean caveGalleryOpened(NGameUI gui) {
        galleryOpened = latchCaveGallery(gui != null ? gui.notices : null, noticeMark, galleryOpened);
    ...
    private Results mineTile(NGameUI gui, MinesweeperSolver solver, Coord tilePos)
        ...
        NUtils.addTask(new NTask() {
            ...
            public boolean check() {
                ...
                if (caveGalleryOpened(gui)) return true;
                return ticks++ > 15;
            }
        });
