...
public class HoneyAndWaxCollector implements Action {
    ...
    static boolean hasHoney(long attr) {
        return (attr & HONEY_BIT) != 0;
    }

    static boolean hasWax(long attr) {
        return (attr & WAX_BIT) != 0;
    }

    private boolean hasHoney(Gob hive) {
        return hasHoney(hive.ngob.getModelAttribute());
    }

    private boolean hasWax(Gob hive) {
        return hasWax(hive.ngob.getModelAttribute());
    }
