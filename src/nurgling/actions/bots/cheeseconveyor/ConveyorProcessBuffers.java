...
/**
 * Copy of {@link nurgling.actions.ProcessCheeseFromBufferContainers} for the Cheese Conveyor bot:
 * slicing and routing follow the conveyor orders ({@link ConveyorOrdersManager#sliceOrder},
 * {@link ConveyorOrdersManager#orderFor}) instead of the Cheese Production Bot's orders.
 ...
 */
public class ConveyorProcessBuffers implements Action {
    private final ConveyorSlicer slicer;
    ...
    public ConveyorProcessBuffers(ConveyorOrdersManager ordersManager, Map<CheeseBranch.Place, Integer> rackCapacity, Map<CheeseBranch.Place, Boolean> bufferEmptinessMap) {
        this.slicer = new ConveyorSlicer();
        ...
        for (Gob staleGob : containers) {
            ...
            while (true) {
                ...
                for (WItem tray : trays) {
                    if (ordersManager.sliceOrder(CheeseUtils.getContentName(tray), place) != null) {
                        readyTray = tray;
                        break;
                    }
                }
                ...
                for (WItem inventoryTray : inventoryTrays) {
                    ConveyorOrder sliceOrder = ordersManager.sliceOrder(CheeseUtils.getContentName(inventoryTray), place);
                    if (sliceOrder != null) {
                        slicer.sliceCheese(gui, inventoryTray, sliceOrder);
                        ordersNeedSaving = true;
    ...
    private CheeseCollectionResult collectCheeseLocationsFromContainers(NGameUI gui, ArrayList<Gob> containers, CheeseBranch.Place place) throws InterruptedException {
        Map<String, ArrayList<CheeseLocation>> unassignedCheeseByType = new HashMap<>();
        ...
        for (Gob staleGob : containers) {
            ...
            ArrayList<WItem> trays = CheeseInventoryOperations.getCheeseTraysFromContainer(gui, bufferContainer);
            for (WItem tray : trays) {
                if (shouldMoveToNextStage(tray, place)) {
                    String cheeseType = CheeseUtils.getContentName(tray);
                    if (cheeseType != null)
                        unassignedCheeseByType.computeIfAbsent(cheeseType, k -> new ArrayList<>())
        ...
        Map<String, Map<CheeseBranch.Place, ArrayList<CheeseLocation>>> cheeseByDestination = new HashMap<>();
        for (Map.Entry<String, ArrayList<CheeseLocation>> entry : unassignedCheeseByType.entrySet()) {
            int offset = 0;
            for (Map.Entry<CheeseBranch.Place, Integer> route :
                    ordersManager.destinationsFor(entry.getKey(), place, entry.getValue().size()).entrySet()) {
                int end = Math.min(entry.getValue().size(), offset + route.getValue());
                if (end > offset) {
                    cheeseByDestination.computeIfAbsent(entry.getKey(), k -> new HashMap<>())
                            .put(route.getKey(), new ArrayList<>(entry.getValue().subList(offset, end)));
                    offset = end;
                }
            }
        }
        return new CheeseCollectionResult(cheeseByDestination);
    }
    ...
    private void processCollectedCheeseByType(NGameUI gui, CheeseCollectionResult collectionResult, NArea area, CheeseBranch.Place place) throws InterruptedException {
        for (Map.Entry<String, Map<CheeseBranch.Place, ArrayList<CheeseLocation>>> typeEntry : collectionResult.cheeseByDestination.entrySet()) {
            String cheeseType = typeEntry.getKey();
            for (Map.Entry<CheeseBranch.Place, ArrayList<CheeseLocation>> route : typeEntry.getValue().entrySet()) {
                CheeseBranch.Place destination = route.getKey();
                ArrayList<CheeseLocation> cheeseLocations = route.getValue();
                ...
                // Collect only the trays planned for this destination before moving the next route.
                collectCheeseFromContainersToInventory(gui, cheeseLocations, destination, cheeseType, area, place,
                        cheeseLocations.size());
    ...
    private void collectCheeseFromContainersToInventory(NGameUI gui, ArrayList<CheeseLocation> cheeseLocations,
                                                        CheeseBranch.Place destination, String cheeseType, NArea area,
                                                        CheeseBranch.Place place, int plannedTrays) throws InterruptedException {
        // Calculate total destination capacity limits once
        int destinationCapacity = calculateDestinationCapacity(gui, destination);
        int alreadyMovedToDestination = traysMovedToAreas.getOrDefault(destination, 0);
        int remainingDestinationCapacity = Math.min(Math.max(0, destinationCapacity - alreadyMovedToDestination), plannedTrays);
    ...
    private int takeCheeseFromSingleContainer(NGameUI gui, Gob containerGob, String cheeseType, int maxToTake, CheeseBranch.Place currentPlace) throws InterruptedException {
        ...
        for (WItem tray : availableTrays) {
            ...
            if (shouldMoveToNextStage(tray, currentPlace)) {
    ...
    private void placeCheeseOnRacksWithOrderUpdates(NGameUI gui, ArrayList<Gob> availableRacks, CheeseBranch.Place destination,
        ...
        for (Gob rackGob : availableRacks) {
            ...
            int traysPlaced = placeTraysOnSingleRack(gui, rack, cheeseTrays, cheeseType, fromPlace, destination);
    ...
    private int placeTraysOnSingleRack(NGameUI gui, Container rack, ArrayList<WItem> cheeseTrays,
                                       String cheeseType, CheeseBranch.Place fromPlace,
                                       CheeseBranch.Place destination) throws InterruptedException {
        ...
        if (traysToPlace > 0 && ordersManager != null && cheeseType != null && fromPlace != null) {
            updateOrdersAfterCheeseMovement(cheeseType, traysToPlace, fromPlace, destination);
            ordersManager.save();
        }
    ...
    /**
     * Aged cheese that is not sliced here moves on to its next stage.
     */
    private boolean shouldMoveToNextStage(WItem tray, CheeseBranch.Place place) {
        return ordersManager.sliceOrder(CheeseUtils.getContentName(tray), place) == null
                && CheeseUtils.isCheeseReadyToMove(tray, place);
    }

    /**
     * Move the stage counters of the order these trays belong to.
     */
    private void updateOrdersAfterCheeseMovement(String cheeseType, int movedCount, CheeseBranch.Place fromPlace,
                                                 CheeseBranch.Place destination) {
        if (ordersManager.advanceMoved(cheeseType, fromPlace, destination, movedCount))
            ordersNeedSaving = true;
    }
    ...
    private static class CheeseCollectionResult {
        public final Map<String, Map<CheeseBranch.Place, ArrayList<CheeseLocation>>> cheeseByDestination;

        public CheeseCollectionResult(Map<String, Map<CheeseBranch.Place, ArrayList<CheeseLocation>>> cheeseByDestination) {
            this.cheeseByDestination = cheeseByDestination;
        }
    }
}
