...
/**
 * Copy of {@link nurgling.actions.MoveReadyCheeseToBuffers} for the Cheese Conveyor bot: finished
 * cheese is taken off the racks when a conveyor order wants it sliced.
 ...
 */
public class ConveyorMoveReadyToBuffers implements Action {
    ...
    public ConveyorMoveReadyToBuffers(ArrayList<Container> racks, ArrayList<Container> buffers, CheeseBranch.Place place, ConveyorOrdersManager ordersManager) {
        this.racks = racks;
    ...
    public ResultWithCapacity runWithCapacity(NGameUI gui) throws InterruptedException {
        ...
        for (Container rack : racksToProcess) {
            ...
            while (!new TakeReadyCheeseFromRack(rack, place, rackCapacities, ordersManager).run(gui).isSuccess) {
    ...
    private static class TakeReadyCheeseFromRack implements Action {
        ...
        public TakeReadyCheeseFromRack(Container rack, CheeseBranch.Place place, Map<Container, Integer> capacityMap, ConveyorOrdersManager ordersManager) {
            this.rack = rack;
            this.place = place;
        ...
        public Results run(NGameUI gui) throws InterruptedException {
            ...
            for (WItem tray : allTrays) {
                if (CheeseUtils.isCheeseReadyToMove(tray, place) || ordersManager.sliceOrder(CheeseUtils.getContentName(tray), place) != null) {
                    readyTrays.add(tray);
                }
            }
