...
/**
 * Copy of {@link nurgling.actions.ClearRacksAndRecordCapacity} for the Cheese Conveyor bot: decides
 * what to take off the racks from the conveyor orders instead of the Cheese Production Bot's orders.
 ...
 */
public class ConveyorClearRacks implements Action {
    ...
    public ConveyorClearRacks(ConveyorOrdersManager ordersManager) {
     ...
     * Uses ConveyorMoveReadyToBuffers for efficient batch processing
    ...
    private int clearReadyCheeseFromArea(NGameUI gui, NArea area, CheeseBranch.Place place) throws InterruptedException {
        ...
        ConveyorMoveReadyToBuffers moveAction = new ConveyorMoveReadyToBuffers(racks, buffers, place, ordersManager);
        ConveyorMoveReadyToBuffers.ResultWithCapacity result = moveAction.runWithCapacity(gui);
     ...
     * Uses the same empty-container check as ConveyorProcessBuffers
     * @param gui Game UI
    ...
    private boolean checkBufferEmptinessForArea(NGameUI gui, NArea area, CheeseBranch.Place place) throws InterruptedException {
        ...
        // Check each buffer using the same empty-container check as ConveyorProcessBuffers
