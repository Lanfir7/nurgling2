...
public class VillagersWindow extends Window {
    ...
    public VillagersWindow() {
        ...
        add(new Label("Invite:"), new Coord(UI.scale(5), y + UI.scale(3)));
        ...
        Button copyButton = add(new Button(UI.scale(90), "Copy") {
            public void click() {
                ...
                if (!inviteOut.text().isEmpty() && DbClipboard.copy(inviteOut.text()))
                    setStatus("Invite copied. Send it to that player privately - it is a password.",
                              Color.YELLOW);
            }
    ...
    private void addVillager() {
        ...
        service().addAsync(name, access)
            .thenAccept(account -> {
                pendingInvite = buildInvite(account, access);
                setStatus("Added " + account.name + ". Copy the invite below and send it privately.",
                          Color.GREEN);
                busy = false;
                refresh();
            })
    ...
    private void resetPassword(String name) {
        ...
        service().resetPasswordAsync(name)
            .thenAccept(account -> {
                pendingInvite = buildInvite(account, VillagerService.ACCESS_MEMBER);
                setStatus("New password for " + account.name
                        + ". Their old invite no longer works.", Color.GREEN);
                busy = false;
                refresh();
            })
            .exceptionally(e -> {
    ...
    private void revoke(String name) {
        if (!ready() || busy)
            return;
        busy = true;
        service().revokeAsync(name)
            .thenAccept(v -> {
                /* Said plainly rather than implied: revoking stops future access and nothing else.
                 * Whatever they synced is on their disk already. */
                setStatus("Removed " + name + ". They keep whatever they already synced to their PC.",
                          Color.YELLOW);
                busy = false;
                refresh();
            })
            .exceptionally(e -> {
                setStatus("Could not remove " + name + ": " + rootMessage(e), Color.ORANGE);
                busy = false;
                return null;
            });
    }
    ...
    private static String buildInvite(VillagerService.NewAccount account, String access) {
        DbSettings s = DbSettings.fromConfig();
        String village = String.valueOf(NConfig.get(NConfig.Key.dbVillage));
    ...
    private static String currentUser() {
        return (NCore.databaseManager == null) ? "" : NCore.databaseManager.getConnectedUser();
    }
