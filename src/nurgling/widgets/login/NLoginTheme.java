...
public final class NLoginTheme {
    ...
    public static String ago(long when) {
        ...
        if (min < 1) return (L10n.get("login.ago.now"));
        if (min < 60) return (L10n.get("login.ago.min", min));
        long h = min / 60;
        if (h < 24) return (L10n.get("login.ago.hour", h));
        long d = h / 24;
        if (d < 7) return (L10n.get("login.ago.day", d));
