...
public class RecipeTable extends Widget {
    ...
    /** Star clicks go through {@link Actions#toggleFavorite} so NCookBook can guard dbReady/ensureFavorites. */
    static void starToggle(Actions actions, CookbookRow row) {
        if(actions != null)
    ...
    class RowWidget extends Widget {
        ...
        private Tex ingredientTip() {
            if(ingTip == null) {
                ...
                Map<String, Double> woods = CookbookRow.smokingWoods(row.recipe);
                if(!woods.isEmpty()) {
                    List<String> woodTips = new ArrayList<>();
                    for(Map.Entry<String, Double> e : woods.entrySet())
                        woodTips.add(RichText.Parser.quote(e.getKey()) + " (" + Utils.odformat2(e.getValue(), 2) + "%)");
                    if(sb.length() > 0)
                        sb.append('\n');
                    sb.append("$col[232,163,61]{").append(RichText.Parser.quote(L10n.get("cookbook.smoked_with")))
                      .append(' ').append(String.join(", ", woodTips)).append('}');
                } else if(!row.woodText.isEmpty()) {
                    if(sb.length() > 0)
                        sb.append('\n');
                    sb.append("$col[232,163,61]{").append(RichText.Parser.quote(L10n.get("cookbook.smoked_with")))
                      .append(' ').append(RichText.Parser.quote(row.woodText)).append('}');
                }
                if(sb.length() == 0)
                    return null;
