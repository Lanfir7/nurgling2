...
import nurgling.widgets.cookbook.CookbookImportReload;
 ...
 * read, off the UI thread, when the window opens or Reload is pressed. Favourites stay on
 * {@link FavoriteRecipeManager}. JSON import stays on {@link ReadJsonAction}.
 */
public class NCookBook extends Window implements RecipeTable.Actions {
    ...
    private volatile boolean pendingImportReload = false;
    ...
    private void ensureFavorites() {
        DatabaseManager db = db();
        if(db == null)
            return;
        if(favoriteManager == null)
            favoriteManager = new FavoriteRecipeManager(db);
        model.setFavoritePort(CookbookModel.asyncManagerPort(favoriteManager, db));
    }
    ...
    public void tick(double dt) {
        ...
        if(CookbookImportReload.shouldStartReload(pendingImportReload, dbReady(), loader != null)) {
            reload();
            /* Clear only after enqueue actually assigned a loader; otherwise keep pending. */
            pendingImportReload = (loader == null);
        }
    ...
    private void importFoodInfo() {
        java.awt.EventQueue.invokeLater(() -> {
            ...
            BotExecutor.runAsync("food-info2_download", new Action() {
                @Override
                public Results run(NGameUI gui) throws InterruptedException {
                    Results results = new ReadJsonAction(path).run(gui);
                    requestReloadAfterImport(results);
                    return results;
                }
            });
        });
    ...
    void requestReloadAfterImport(Results results) {
        if(CookbookImportReload.shouldReload(results))
    ...
    public void rowMenu(CookbookRow row, Widget at) {
        ...
        String[] opts = {L10n.get(recipe.isFavorite() ? "cookbook.remove_from_favorites" : "cookbook.add_to_favorites")};
