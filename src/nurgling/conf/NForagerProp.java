...
public class NForagerProp implements JConf {
    ...
    public NForagerProp(HashMap<String, Object> values) {
        ...
        // Same presence rule as actionsProfiles: a present empty "guardingProfiles":{} is an
        // intentional wipe and must not resurrect legacy per-preset onAnimalAction/onPlayerAction.
        boolean hadGuardingProfilesKey = values.get("guardingProfiles") != null;
        guardingProfiles = new HashMap<>();
        if (hadGuardingProfilesKey) {
        ...
        if (!guardingProfiles.containsKey(currentGuardingProfile)) {
            if (guardingProfiles.isEmpty()) {
                guardingProfiles.put("Default", GuardingProfile.withDefaults());
                currentGuardingProfile = "Default";
            } else {
