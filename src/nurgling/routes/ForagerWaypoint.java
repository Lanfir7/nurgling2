...
public class ForagerWaypoint {
    ...
    // Canonical: "nothing" (continue), "break" (stop), "logout", "travel hearth".
    public String onStepsFailAction = FAIL_NOTHING;

    public static final String FAIL_NOTHING = "nothing";
    public static final String FAIL_BREAK = "break";
    public static final String FAIL_LOGOUT = "logout";
    public static final String FAIL_TRAVEL_HEARTH = "travel hearth";
    public static final String[] FAIL_ACTIONS = {FAIL_NOTHING, FAIL_BREAK, FAIL_LOGOUT, FAIL_TRAVEL_HEARTH};

    public static String normalizeOnStepsFailAction(String raw) {
        if (raw == null)
            return FAIL_NOTHING;
        String trimmed = raw.trim();
        if (trimmed.isEmpty())
            return FAIL_NOTHING;
        String compact = trimmed.toLowerCase(java.util.Locale.ROOT).replace('_', '-').replace(' ', '-');
        if (FAIL_BREAK.equals(compact))
            return FAIL_BREAK;
        if (FAIL_LOGOUT.equals(compact))
            return FAIL_LOGOUT;
        if ("travel-hearth".equals(compact) || "travelhearth".equals(compact) || "hearth".equals(compact))
            return FAIL_TRAVEL_HEARTH;
        if (FAIL_NOTHING.equals(compact))
            return FAIL_NOTHING;
        return FAIL_NOTHING;
    }

    /** True when waypoint-step failure should stop the Forager rather than continue the route. */
    public static boolean stopsRouteOnStepsFail(String raw) {
        return !FAIL_NOTHING.equals(normalizeOnStepsFailAction(raw));
    }

    /** True when the fail action is logout or travel-hearth (GuardOutcome), not a no-op break/nothing. */
    public static boolean performsGuardOutcomeOnStepsFail(String raw) {
        String action = normalizeOnStepsFailAction(raw);
        return FAIL_LOGOUT.equals(action) || FAIL_TRAVEL_HEARTH.equals(action);
    }
    ...
    public ForagerWaypoint(JSONObject json) {
        ...
        if (json.has("onStepsFailAction")) {
            this.onStepsFailAction = normalizeOnStepsFailAction(json.getString("onStepsFailAction"));
        }
    ...
    public JSONObject toJson() {
        ...
        if (!FAIL_NOTHING.equals(normalizeOnStepsFailAction(onStepsFailAction))) {
            json.put("onStepsFailAction", normalizeOnStepsFailAction(onStepsFailAction));
        }
