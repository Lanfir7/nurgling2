...
public class TodoStore {
    ...
    /** True once the current pending batch has been copied into the local recovery payload. */
    private boolean pendingRecoveryStaged = false;
    ...
    private void ensureMode() {
        ...
        synchronized (lock) {
            ...
            if (m == Mode.LOCAL || wasLocal) {
                /* A shared edit may still only live in pending when the user turns the database
                 * off. Preserve a copy in the local file before clearing DB state; it is later
                 * imported as new rows, never applied over a newer database row. */
                boolean hasPendingRecovery = !wasLocal && m == Mode.LOCAL && !pending.isEmpty();
                if (hasPendingRecovery) {
                    preservePendingAsLocalLocked();
                    if (!saveFileLocked())
                        return;
                }
                if (fileDirty)
    ...
    public void uploadLocal() {
        synchronized (lock) {
            ...
            for (int i = 0; i < asideItems.length(); i++) {
                ...
                n.touchedBy = me();
                n.touchedAt = now;
                queuePendingLocked(TodoOp.createItem(n));
            }
            asideLists = null;
            asideItems = null;
            markFileDirtyLocked();
    ...
    private void editItemLocked(int id, Set<TodoOp.Field> fields, Consumer<TodoItem> fn) {
        ...
        if (cur.listId == TodoList.PERSONAL) {
            ...
            queuePendingLocked(TodoOp.editItem(id, fields, stamped, cur.copy()));
        }
    }
    ...
    private void editListLocked(int id, Consumer<TodoList> fn) {
        ...
        if (loadedMode == Mode.LOCAL) {
            ...
            queuePendingLocked(TodoOp.editList(id, stamped));
        }
    }
    ...
    private int createListLocked(String name) {
        ...
        if (loadedMode == Mode.LOCAL) {
            ...
            queuePendingLocked(TodoOp.createList(l));
        }
        return l.id;
    }
    ...
    private void putNewItemLocked(TodoItem it) {
        if (it.listId == TodoList.PERSONAL) {
            ...
            queuePendingLocked(TodoOp.createItem(it.copy()));
        }
    }
        ...
        try {
            if (needBulk) {
                ...
                synchronized (lock) {
                    ...
                    if (!writable) {
                        preservePendingAsLocalLocked();
                        if (!pending.isEmpty() && !saveFileLocked())
                            return;
                        readOnly = true;
                        pending.clear();
                        pendingRecoveryStaged = false;
    ...
    private void queuePendingLocked(TodoOp op) {
        pending.add(op);
        pendingRecoveryStaged = false;
    }

    /**
     * Converts pending shared changes into fresh local rows. This is a recovery copy, rather than an
     * operation journal: after a database permission failure or a DB-to-local switch it can only be
     * imported as new rows, so it can never overwrite another villager's newer database edit. Pending
     * deletes become clearly labelled recovery rows: silently retrying a delete could erase a newer edit.
     */
    private void preservePendingAsLocalLocked() {
        if (pending.isEmpty() || pendingRecoveryStaged)
            return;

        Map<Integer, TodoList> lists = new HashMap<>(baseLists);
        Map<Integer, TodoItem> items = new HashMap<>(baseItems);
        Set<Integer> affectedLists = new HashSet<>();
        Set<Integer> affectedItems = new HashSet<>();
        for (TodoOp op : pending) {
            if (op.isList) {
                TodoList list = op.newList == null ? lists.get(op.id) : op.newList;
                if (list == null)
                    continue;
                list = list.copy();
                if (op.newList == null)
                    op.listFn.accept(list);
                lists.put(op.id, list);
                affectedLists.add(op.id);
            } else {
                TodoItem item = op.newItem == null ? items.get(op.id) : op.newItem;
                if (item == null)
                    continue;
                item = item.copy();
                if (op.newItem == null)
                    op.itemFn.accept(item);
                items.put(op.id, item);
                affectedItems.add(op.id);
                affectedLists.add(item.listId);
            }
        }

        if (asideLists == null)
            asideLists = new JSONArray();
        if (asideItems == null)
            asideItems = new JSONArray();
        Set<Integer> used = asideIds(asideLists, asideItems);
        Map<Integer, Integer> listIds = new HashMap<>();
        for (Integer id : affectedLists) {
            TodoList list = lists.get(id);
            if (list == null || list.isPersonal())
                continue;
            TodoList copy = list.copy();
            if (copy.deleted) {
                copy.deleted = false;
                copy.name = clip("Delete requested: " + copy.name, TodoList.MAX_NAME);
            }
            copy.id = freshAsideId(used);
            copy.version = 0;
            listIds.put(id, copy.id);
            asideLists.put(listJson(copy));
        }
        for (Integer id : affectedItems) {
            TodoItem item = items.get(id);
            Integer listId = item == null ? null : listIds.get(item.listId);
            if (item == null || listId == null)
                continue;
            TodoItem copy = item.copy();
            if (copy.deleted) {
                copy.deleted = false;
                copy.title = clip("Delete requested: " + copy.title, TodoItem.MAX_TITLE);
                copy.notes = "Recovery copy: deletion was not sent to the database. Review and delete manually."
                    + (copy.notes.isEmpty() ? "" : "\n\n" + copy.notes);
            }
            copy.id = freshAsideId(used);
            copy.listId = listId;
            copy.version = 0;
            asideItems.put(itemJson(copy));
        }
        uploadDismissed = false;
        markFileDirtyLocked();
        pendingRecoveryStaged = true;
    }

    private static Set<Integer> asideIds(JSONArray lists, JSONArray items) {
        Set<Integer> ids = new HashSet<>();
        for (JSONArray rows : new JSONArray[]{lists, items}) {
            for (int i = 0; i < rows.length(); i++) {
                JSONObject row = rows.optJSONObject(i);
                if (row != null && row.has("id"))
                    ids.add(row.optInt("id"));
            }
        }
        return ids;
    }

    private int freshAsideId(Set<Integer> used) {
        int id;
        do {
            id = rnd.nextInt(Integer.MAX_VALUE - 2) + 2;
        } while (!used.add(id));
        return id;
    }

    /** Called when a database operation reveals that this account cannot write shared rows. */
    ...
    private boolean saveFileLocked() {
        fileDirty = false;
