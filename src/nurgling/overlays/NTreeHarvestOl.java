...
/**
 * Overlay for trees/bushes showing what can be harvested (leaf/seed/fruit),
 * inspired by HerSRC's GobReadyForHarvestInfo.
 *
 * It renders a small combined icon (leaf + fruit/seed), only when the object is mature/ready.
 */
public class NTreeHarvestOl extends NObjectTexLabel {
	/* Same growth conversion constants as HerSRC */
	...
	/**
	 * Resource basename -> invobj icon for "seed"/fruit.
	 * For most trees it is gfx/invobjs/seed-<basename>, but the game is inconsistent.
	 */
	private static final Map<String, String> SEEDS_MAP;

	/**
	 * Resource basename -> invobj icon for leaves (only for specific plants; otherwise leaf flag can be misleading).
	 */
	private static final Map<String, String> LEAVES_MAP;

	/**
	 * Resource basename -> invobj icon for boughs. Only exists for certain trees.
	 * (Icons are species-specific, e.g. gfx/invobjs/bough-spruce)
	 */
	...
	static {
		// Immutable mapping tables; HashMap supports null values (needed for explicit "no icon" cases).
		Map<String, String> seeds = new HashMap<>();
		// Trees (fruit shown instead of seed):
		...
		seeds.put("charredtree", null); // explicitly: no seeds/fruits
		// Bushes:
		seeds.put("ghostpipe", "gfx/invobjs/ghostpipes");
		seeds.put("mastic", "gfx/invobjs/masticfruit");
		seeds.put("poppycaps", "gfx/invobjs/poppycapss"); // Yes, double 's'
		SEEDS_MAP = Collections.unmodifiableMap(seeds);

		Map<String, String> leaves = new HashMap<>();
		// These have to be manually done; otherwise leaf flag can be misleading.
		...
		// Known bough-bearing trees (see VSpec "Tree Bough" category)
	...
	/**
	 * Recompute label/img based on current gob sdt and resource.
	 *
	 * @return true if there is something to display, false otherwise.
	 */
	...
	public boolean tick(double dt) {
		// This overlay is managed externally (added/removed in NGob); keep it unless the gob lost drawable.
	...
	/**
	 * Builds combined icon tex for current gob state, or null if nothing should be shown now.
	 */
	public static TexI computeLabel(Gob gob) {
		...
		// Only for actual tree sprite-kind
		...
		// Growth: same as HerSRC (skip 1, take uint8)
		data.skip(1);
		...
		// Show only for mature (>= 100%) or unknown growth
		if(!(growth == -1 || growth >= 100))
			return null;

		// Flags: read full sdt as little-endian int (Sprite.decnum)
		int sdt = Sprite.decnum(d.sdt.clone());
		String base = res.basename();

		boolean seed = (sdt & 1) != 1;
		boolean leaf = (sdt & 2) != 2;
		// Keep bough logic separate (as before). We don't have a reliable flag yet,
		// so this stays conservative: only for known bough-bearing trees.
		...
		for(BufferedImage part : parts) {
			if(part != null) { hasPart = true; break; }
		}
		if(!hasPart)
			return null;

		// Compose side-by-side with vertical centering (bough icons are taller than seeds).
	...
	/**
	 * Like ItemInfo.catimgsh, but vertically centers each image in the row.
	 */
	...
	private static BufferedImage getIcon(String basename, String type) {
		...
		if("seed".equals(type)) {
			if(SEEDS_MAP.containsKey(basename)) {
				// May be null intentionally (e.g. charredtree)
			...
			// Only certain trees have bough icons; unknown trees return null to avoid useless loading attempts.
		...
		try {
			...
			// Bough icons are tall; keep their intended aspect by scaling to 26x52.
		...
		// ConcurrentHashMap forbids null values; store Optional instead.
