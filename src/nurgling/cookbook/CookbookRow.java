...
public final class CookbookRow {
    ...
    public CookbookRow(Recipe recipe, Map<SpiceCalc.Spice, Double> spices) {
        this.recipe = recipe;
        Map<SpiceCalc.Spice, Double> applied = (spices == null)
                ? Collections.<SpiceCalc.Spice, Double>emptyMap() : spices;
        SpiceCalc.Result res = SpiceCalc.apply(fepsOf(recipe), recipe == null ? 0 : recipe.getHunger(), applied);
        ...
        Map<String, Recipe.IngredientInfo> ingMap = (recipe == null || recipe.getIngredients() == null)
                ? Collections.<String, Recipe.IngredientInfo>emptyMap()
                : recipe.getIngredients();
        List<Map.Entry<String, Recipe.IngredientInfo>> ings = new ArrayList<>(ingMap.entrySet());
        ings.sort((a, b) -> {
        ...
        for(String wood : smokingWoods(recipe).keySet()) {
            if(wt.length() > 0)
                wt.append(", ");
            wt.append(wood);
            sources.add(wood);
    ...
    public String name() {
        return (recipe == null || recipe.getName() == null) ? "" : recipe.getName();
    }
    ...
    /** Smoking woods from the recipe; empty when none or the recipe is missing. */
    public static Map<String, Double> smokingWoods(Recipe recipe) {
        if(recipe == null || recipe.getSmokingWoods() == null)
            return Collections.emptyMap();
        return recipe.getSmokingWoods();
    }

    static List<FepValue> fepsOf(Recipe recipe) {
        List<FepValue> ret = new ArrayList<>();
        if(recipe == null || recipe.getFeps() == null)
            return ret;
        for(Map.Entry<String, Recipe.Fep> e : recipe.getFeps().entrySet())
            ret.add(FepValue.of(e.getKey(), e.getValue() == null ? 0 : e.getValue().val));
        return ret;
    }
